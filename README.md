...
3-3 用户模板管理组成员：蔡世强、侯锦瑞、徐崇耀、张楚唯、潘家杰、王杰。

主要负责内容：

- 用户账号体系：本地账号初始化、注册、登录、退出、资料更新、密码修改和登录态管理。
- 扫码登录：通过 Vite 开发中间件建立扫码会话，实现同一局域网内手机确认、电脑登录。
- 历史版本：按用户名和模板 ID 隔离保存简历历史快照，支持历史版本标题生成、预览、恢复、删除和旧数据迁移。
- 模板管理：加载模板列表、展示模板市场、切换当前模板并保存模板设置。
- 高保真导出：在简历预览页提供 PDF 导出和 DPI 清晰度选择。
...
- 本项目当前用户数据、简历数据、历史版本和设置数据主要保存在浏览器本地存储中。
- 新拉取代码后如出现依赖相关报错，请先执行 `npm install`。
- 若模板切换后预览异常，可清理浏览器本地缓存或将 `resumeData.resumeSetting.currentTemplate` 重置为有效模板 ID。
